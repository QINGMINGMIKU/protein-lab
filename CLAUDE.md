...
- 本地 commit 随意，不影响远程。
- ⚠️ **推 tag 即发布**：`.github/workflows/build.yml` 的触发是 `on: push: tags: ['v*']`——推送任何 `v*` tag 都会在 Win/macOS 双平台构建 onedir zip 并**自动创建 GitHub Release**（`softprops/action-gh-release` + `contents: write`）。要只留本地 tag 不发布，就**别推 tag**。
...
- v0.1.4 ✓ 已打 tag (2026-08-28, `6236305`) — 研究脉络 UI：observation 创建/编辑/渲染 + 结论编辑多选支持实验（**在朋友 UI PR 落地后做**，避免做一套被重写）
...
- v0.1.5 ✓ 已打 tag (2026-09-20) — **快照发布**：涵盖 2026-09-16→09-18 三条「不占版本号」改动（酶活三段式契约补齐 / 实验归档入口收口 / 研究脉络挂载自由化）+ 此前 `researcher` 线内容；tag 推送触发 CI 双平台构建并自动建 Release（**已发布 2026-09-21**：`protein-lab-windows.zip` 99.8MB / `protein-lab-macos.zip` 82.4MB，CI run `35568065407` 双 job 全绿；tag 建于 2026-09-20，Release 跨零点于次日）。
- v0.1.6 — Comparison：WT vs variant 多实验横切对比 + 判断辅助（Workbench 差异化核心）+ `used_sample_from` 采样来源标注
